...
static func create_step_from_node(node: Dictionary) -> PipelineStep:
	...
	match node_type:
		...
		"show_level_failure":
			return _create_show_level_failure_step(node)
		"cutscene":
...
static func _create_show_level_failure_step(node: Dictionary) -> PipelineStep:
	var level_num = node.get("level_number", 0)
	return ShowLevelFailureStep.new(level_num)
...
static func _create_conditional_step(node: Dictionary) -> PipelineStep:
	...
	return ConditionalStep.new(cond, t_branch, f_branch)
