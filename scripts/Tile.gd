...
# Local cache for node resolver API
var _nr_api_cache = null
func _nr_api():
    if _nr_api_cache == null:
        var s = load("res://scripts/helpers/node_resolvers_api.gd")
        if s != null:
            _nr_api_cache = s
    return _nr_api_cache
...
func _resolve_theme_name() -> String:
    # theme manager lookup
    var api = _nr_api()
    var tm = null
    if api != null and api.has_method("_get_tm"):
        tm = api._get_tm()
    if tm == null and has_method("get_tree"):
        var _root_tm = get_tree().root
        if _root_tm:
            tm = _root_tm.get_node_or_null("ThemeManager")
    ...
    for m in candidates:
        if tm and tm.has_method(m):
            var res = tm.call(m)
            if typeof(res) == TYPE_STRING and res != "":
                return res
    for p in candidates:
        if tm and tm.has_method("get"):
            var v = tm.get(p)
...
func update_visual():
    ...
    # Default texture path (must be declared before conditional overrides)
    var texture_path: String = "res://textures/tile_%d.png" % tile_type
    ...
    if theme_manager == null and has_method("get_tree"):
        var _root_tm2 = get_tree().root
        if _root_tm2:
            theme_manager = _root_tm2.get_node_or_null("ThemeManager")
...
func _transform_on_hard_destroy(reveal: Dictionary) -> void:
    ...
    if rtype == "collectible":
        ...
        if board and board.has_method("grid_to_world_position"):
...
func _create_collection_particles() -> void:
    ...
    # Schedule cleanup using a SceneTree timer and connect directly to the particle's queue_free
...
func _create_unmovable_destruction_particles():
    ...
    if _am_local and _am_local.has_method("play_sfx"):
        ...
        if unmovable_type == "glass" or unmovable_type == "wood":
            _am_local.play_sfx("tile_break")  # Glass/wood breaking sound
        else:
            _am_local.play_sfx("tile_match")
...
func _input(event):
    # Skip input on tiles pending deletion — they are still in the tree but no longer valid
    if is_queued_for_deletion():
        return
...
func _on_mouse_entered():
    ...
    var gm = _get_gm()
    if not is_falling and not (gm and gm.processing_moves) and sprite and sprite.texture:
...
func handle_click():
    ...
    var gm = _get_gm()
    if is_falling or (gm and gm.processing_moves):
        print("Click blocked - falling: ", is_falling, " processing: ", (gm.processing_moves if gm else false))
        return
...
# New helper to resolve GameManager safely
func _get_gm():
    var api = _nr_api()
    if api != null and api.has_method("_get_gm"):
        var gm = api._get_gm()
        if gm != null:
            return gm

    # Fallback to autoload/root lookup
    if has_method("get_tree"):
        var rt = get_tree().root
        if rt:
            var gm2 = rt.get_node_or_null("GameManager")
            if gm2:
                return gm2
    return null

# Local audio resolver helper
func _am():
    var api = _nr_api()
    var am = null
    if api != null and api.has_method("_get_am"):
        am = api._get_am()
    if am != null:
        return am
    # Last resort: global AudioManager
